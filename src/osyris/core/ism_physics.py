...
def read_opacity_table(fname, ngrp):
    ...
    offsets["n"] += 9
    offsets["d"] += 1
    theTable["dens"] = utils.read_binary_data(fmt="%id" % theTable["nx"][1],
                                              content=data,
                                              offsets=offsets,
                                              increment=False)
    offsets["n"] -= 1
    ...
    offsets["d"] += 1
    theTable["tgas"] = utils.read_binary_data(fmt="%id" % theTable["nx"][2],
                                              content=data,
                                              offsets=offsets,
                                              increment=False)
    offsets["n"] -= 1
    ...
    offsets["d"] += 1
    theTable["trad"] = utils.read_binary_data(fmt="%id" % theTable["nx"][3],
                                              content=data,
                                              offsets=offsets,
                                              increment=False)
    offsets["n"] -= 1
...
def get_opacities(dataset, fname, variables=["kappa_p", "kappa_r"]):
    ...
    for i in range(1, ngrp + 1):
        if "radiative_temperature_{}".format(i) not in dataset["hydro"]:
            print("radiative_temperature_{} is not defined. Computing it now...".format(
                i),
                  end="")
            rc = 1 * units("radiation_constant")
            dataset["hydro"]["radiative_temperature_{}".format(i)] = ((
                dataset["hydro"]["radiative_energy_{}".format(i)] / rc)**.25).to("K")
            print(" done!")
        # check if radiative temperature out of grid bounds:
        rad_temp = np.copy(
            dataset["hydro"]["radiative_temperature_{}".format(i)].values)
        is_out_of_bounds = np.log10(rad_temp) < np.min(
            dataset.meta["opacity_table"]["grid"][2])
        if np.any(is_out_of_bounds):
            print(
                "WARNING: Radiative temperature for group {} is out of bounds!".format(
                    i))
            # proceed to fill those values with min grid trad
            rad_temp[is_out_of_bounds] = 10**np.min(
                dataset.meta["opacity_table"]["grid"][2])
        pts = np.array([
        ...
        for j, var in enumerate(variables):
            ...
            vals = ism_interpolate(dataset.meta["opacity_table"],
                                   dataset.meta["opacity_table"]["all_kappa_r"][i - 1],
                                   pts)
            print(" done!")
...
def get_eos(dataset,
            fname,
            variables=[
                "rho_eos", "ener_eos", "temp_eos", "pres_eos", "s_eos", "cs_eos",
                "xH_eos", "xH2_eos", "xHe_eos", "xHep_eos"
            ]):
    """
...
def get_masson_table(theTable, data, ndims):
    ...
    # Get table dimensions
    nx, ny, nz = np.array(
        utils.read_binary_data(fmt="%ii" % ndims, content=data, increment=False))
    ...
    if ndims == 4:
        ...
        offsets["n"] += theTable["nx"][1]
        offsets["d"] += 1
        theTable["ionx"] = utils.read_binary_data(fmt="%id" % theTable["nx"][2],
                                                  content=data,
                                                  offsets=offsets,
                                                  increment=False)
        offsets["n"] -= 1
    ...
    offsets["n"] += theTable["nx"][-2]
    offsets["d"] += 1
    theTable["bmag"] = utils.read_binary_data(fmt="%id" % theTable["nx"][-1],
                                              content=data,
                                              offsets=offsets,
                                              increment=False)
    offsets["n"] -= 1
...
def compute_resistivities(resistivite_chimie_x, nx, ndims):
    ...
    if nbins_grains == 1:
        ...
        for i in range(nbins_grains):
            r_g[i] = a_min * zeta**(-float(i + 1) / fnb) * np.sqrt(
                Lp1 / Lp3 * (1.0 - zeta**(Lp3 / fnb)) / (1.0 - zeta**(Lp1 / fnb)))
    q = np.zeros((nx[0]))
    m = np.zeros((nx[0]))
    m_g = np.zeros((nx[0]))
    q[:] = e
        ...
        for iB in range(bchimie):
            for iT in range(tchimie):
                for iH in range(nchimie):
                    B = Barray[iB]
                    nh = resistivite_chimie_x[0, iH, iT,
                                              iX]  # density (.cc) of current point
                    T = resistivite_chimie_x[1, iH, iT, iX]
                    xi = resistivite_chimie_x[2, iH, iT, iX]
                    for i in range(nion):
                        if i == 0:  # electron
                            sigv = 3.16e-11 * (np.sqrt(8.0 * kb * 1.0e-7 * T /
                                                       (np.pi * me * 1.0e-3)) *
                                               1.0e-3)**1.3
                            tau_sn[i] = 1.0 / 1.16 * (m[i] + 2.0 * mp) / (
                                2.0 * mp) * 1.0 / (nh / 2.0 * sigv)
                        else:  # ions
                            muuu = m[i] * 2.0 * mp / (m[i] + 2.0 * mp)
                            if (i == 1) or (i == 2):
                                sigv = 2.4e-9 * (np.sqrt(8.0 * kb * 1.0e-7 * T /
                                                         (np.pi * muuu * 1.0e-3)) *
                                                 1.0e-3)**0.6
                            elif i == 3:
                                sigv = 2.0e-9 * (np.sqrt(8.0 * kb * 1.0e-7 * T /
                                                         (np.pi * muuu * 1.0e-3)) *
                                                 1.0e-3)**0.15
                            elif i == 4:
                                sigv = 3.89e-9 * (np.sqrt(8.0 * kb * 1.0e-7 * T /
                                                          (np.pi * muuu * 1.0e-3)) *
                                                  1.0e-3)**(-0.02)
                            else:
                                sigv = 1.69e-9
                            tau_sn[i] = 1.0 / 1.14 * (m[i] + 2.0 * mp) / (
                                2.0 * mp) * 1.0 / (nh / 2.0 * sigv)
                        omega[i] = q[i] * B / (m[i] * clight)
                        sigma[i] = resistivite_chimie_x[i + 3, iH, iT, iX] * nh * (
                            q[i])**2 * tau_sn[i] / m[i]
                        gamma_zeta[i] = 1.0
                        gamma_omega[i] = 1.0
                    for i in range(nbins_grains):
                        # g+
                        tau_sn[nion + 1 + 3 * i] = 1.0 / 1.28 * (m_g[i] + 2.0 * mp) / (
                            2.0 * mp) * 1.0 / (nh / 2.0 * (np.pi * r_g[i]**2 *
                                                           (8.0 * kb * T /
                                                            (np.pi * 2.0 * mp))**0.5))
                        omega[nion + 1 +
                              3 * i] = q[nion + 1 + 3 * i] * B / (m_g[i] * clight)
                        # g-
                        tau_sn[nion + 2 + 3 * i] = tau_sn[nion + 1 + 3 * i]
                        omega[nion + 2 +
                              3 * i] = q[nion + 2 + 3 * i] * B / (m_g[i] * clight)
                        sigma[nion + 1 + 3 * i] = resistivite_chimie_x[
                            nion + 4 + 3 * i, iH, iT,
                            iX] * nh * (q[nion + 1 + 3 * i]**
                                        2) * tau_sn[nion + 1 + 3 * i] / m_g[i]
                        sigma[nion + 2 + 3 * i] = resistivite_chimie_x[
                            nion + 5 + 3 * i, iH, iT,
                            iX] * nh * (q[nion + 2 + 3 * i]**
                                        2) * tau_sn[nion + 2 + 3 * i] / m_g[i]
                    sigP = 0.0
                    sigO = 0.0
                    sigH = 0.0
                    for i in range(nx[0]):
                        ...
                        sigH = sigH - sigma[i] * omega[i] * tau_sn[i] / (
                            1.0 + (omega[i] * tau_sn[i])**2)
                    eta_ohm[iH, iT, iX, iB] = np.log10(1.0 / sigP)  # Ohmic
                    eta_ad[iH, iT, iX, iB] = np.log10(sigO / (sigO**2 + sigH**2) -
                                                      1.0 / sigP)  # Ambipolar
                    eta_hall[iH, iT, iX,
                             iB] = np.log10(abs(sigH / (sigO**2 + sigH**2)))  # Hall
...
def read_marchand_table(data, ndims):
    ...
    offsets["i"] += ndims
    offsets["d"] += 1
    resistivite_chimie_x = np.reshape(utils.read_binary_data(fmt="%id" %
                                                             (np.prod(nx_read)),
                                                             content=data,
                                                             offsets=offsets),
                                      nx_read,
...
def read_resistivity_table(fname="resistivities_masson2016.bin"):
    ...
    if ndims == 3:
        ...
        nminchimie, nmaxchimie = np.min(resistivite_chimie_x[0]), np.max(
            resistivite_chimie_x[0])
        print("Computing 3D resistivity table...", end="")
        eta_ohm, eta_ad, eta_hall, eta_hsig = compute_resistivities(
            resistivite_chimie_x, nx, ndims)
        print(" done!")
        theTable = {
            "eta_ad": eta_ad,
            "eta_ohm": eta_ohm,
        ...
        tminchimie, tmaxchimie = np.min(resistivite_chimie_x[1]), np.max(
            resistivite_chimie_x[1])
        ximinchimie, ximaxchimie = np.min(resistivite_chimie_x[2]), np.max(
            resistivite_chimie_x[2])
        bminchimie, bmaxchimie = 1e-10, 1e+10
        dens_arr = np.log10(
            np.logspace(np.log10(nminchimie), np.log10(nmaxchimie), eta_ad.shape[0]))
        T_arr = np.log10(
            np.logspace(np.log10(tminchimie), np.log10(tmaxchimie), eta_ad.shape[1]))
        xi_arr = np.log10(
            np.logspace(np.log10(ximinchimie), np.log10(ximaxchimie), eta_ad.shape[2]))
        B_arr = np.log10(
            np.logspace(np.log10(bminchimie), np.log10(bmaxchimie), eta_ad.shape[3]))
...
def get_resistivities(dataset, fname, variables=["eta_ohm", "eta_ad", "eta_hall"]):
    ...
    try:
        ...
        rho_to_nH = dataset.meta["res_table"][
            "scale_dens"] / 2.31  # Default mean atomic weight

    ionisation_rate = np.ones(
        dataset["hydro"].shape) * dataset.meta["res_table"]["ionis_rate"]
    ...
    is_out_of_bounds_temp = np.log10(temp) > np.max(
        dataset.meta["res_table"]["grid"][1])
    ...
    if dataset.meta["res_table"]["ndims"] == 4:
        pts = np.array([
            ...
            np.log10(dataset["hydro"]["B_field"].norm.to("G").values)
        ]).T
    ...
    for var in variables:
        print("Interpolating " + var + "...", end="")
        vals = ism_interpolate(dataset.meta["res_table"],
                               dataset.meta["res_table"][var], pts)
        if var == "eta_hall":
            hall_sign = np.sign(
                ism_interpolate(dataset.meta["res_table"],
                                dataset.meta["res_table"]["eta_hsig"],
                                pts,
                                in_log=True))
